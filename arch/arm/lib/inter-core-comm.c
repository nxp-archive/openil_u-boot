...
void icc_show(void)
{
	...
	printf(
		"mycoreid: %d; ICC_SGI: %d; share_memory_size: %d\n",
		mycoreid, ICC_SGI, ICC_CORE_MEM_SPACE);
	printf(
		"block_unit_size: %d; block number: %d; block_idx: %d\n",
		ICC_BLOCK_UNIT_SIZE, ICC_CORE_BLOCK_COUNT, block_idx);

	for (i = 0; i < CONFIG_MAX_CPUS; i++) {
		printf("\n");
		printf(
			"#ring %d base: %p; dest_core: %d; SGI: %d\n",
			i, ring[i], ring[i]->dest_coreid, ring[i]->interrupt);
		printf(
			"desc_num: %d; desc_base: %p; head: %d; tail: %d\n",
			ring[i]->desc_num, ring[i]->desc, ring[i]->desc_head,
			ring[i]->desc_tail);
		printf(
			"busy_counts: %ld; interrupt_counts: %ld\n",
